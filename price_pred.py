...
dagshub.init(repo_owner='PratyakshRaj', repo_name='MLflow_exp_dagshub', mlflow=True)

mlflow.set_experiment("Price_pred_xg")
mlflow.set_tracking_uri("https://dagshub.com/PratyakshRaj/MLflow_exp_dagshub.mlflow")
...
cat_columns = ['type', 'amenities', 'postcode', 'Area']  # or use your dynamic list
for col in cat_columns:
    f5[col] = f5[col].astype("category")
...
with mlflow.start_run():
    ...
    trained_model= XGBRegressor(max_depth=depth,n_estimators=n_estimators,reg_alpha=0.01,reg_lambda=1,tree_method="hist", enable_categorical=True,verbosity=1).fit(x_train_1,y_train_1)
    ...
    mlflow.log_artifact("model.pkl","XGboost")
